    ...
    <div>
      ...
      {data &&
        slicedData.map((artist: any, i: number) => {
          return (
            <Link to={`${ERoutes.ARTIST}/${artist.slug}`} key={i} className="inline-block p-2 bg-red-400">
              <h1>{artist.name}</h1>
            </Link>
          );
        })}

      <Button onClick={() => setAmountOfDataShown(amountOfDataShown + 12)} disabled={amountOfDataShown >= data?.length}>
        Vis flere
      </Button>
    </div>
  );
